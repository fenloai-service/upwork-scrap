...
def send_notification_resend(proposals: list[dict], stats: dict, dry_run: bool = False):
    ...
    try:
        email_cfg = load_config("email_config", top_level_key="email")
        dashboard_url = email_cfg.get('dashboard_url', 'http://localhost:8501')
    except Exception:
        dashboard_url = 'http://localhost:8501'

    # Build email content
    subject = f"🎯 {len(proposals)} New Upwork Proposals Generated"
    
    html_body = f"""
    <html>
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>🎯 Upwork Proposal Report</h2>
        ...
        <h3>✍️ Proposals ({len(proposals)})</h3>
    """
    ...
    html_body += f"""
        <hr>
        <p><a href="{dashboard_url}" style="color: #14a800; font-weight: 600; text-decoration: none;">🚀 View Full Dashboard</a></p>
